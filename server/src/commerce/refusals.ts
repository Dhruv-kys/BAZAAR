...
const HTTP_STATUS: Partial<Record<RefusalCode, number>> = {
  ...
  BILLING_DETAILS_REQUIRED: 400,
  BILLING_DETAILS_INVALID: 400,
...
export const REFUSAL_CONTRACT: Record<RefusalCode, RefusalSpec> = {
  ...
  BILLING_DETAILS_REQUIRED: { retryable: true, meaning: "The human door issues no payment link without the payer's billing name, email and mobile number. Agents identify themselves by mandate instead and never send this." },
  BILLING_DETAILS_INVALID: { retryable: true, meaning: "The billing details were present but one of them failed validation; the message names the field that has to change." },
